...
type voxFaceSlice[T comparable] struct {
	w, h  int           // Dimensions
	e     T             // The empty value
	f     Facing        // Facing of all faces in this slice
	faces []*voxFace[T] // Collection of faces to generate
}

// newVoxFaceSlice returns a new voxFaceSlice.
func newVoxFaceSlice[T comparable](w, h int, e T, f Facing) *voxFaceSlice[T] {
...
func BuildVoxelMesh[T comparable](v VoxelSource[T], empty T, d verter[T]) {
	width, height, depth := v.Dimensions()
	// Determine if a face is required
	face := func(pos [3]int, f Facing) bool {
		np := [3]int{}
	...
	// N/S faces sweeps
	for f := North; f <= South; f++ {
		for iz := 0; iz < depth; iz++ {
			// Build face slice
			s := newVoxFaceSlice(width, height, empty, f)
	...
	for f := East; f <= West; f++ {
		for ix := 0; ix < width; ix++ {
			// Build face slice
			s := newVoxFaceSlice(depth, height, empty, f)
			for iy := 0; iy < height; iy++ {
				for iz := 0; iz < depth; iz++ {
					vv := v.Get(ix, iy, iz)
					if vv == empty {
	...
	for f := Top; f <= Bottom; f++ {
		for iy := 0; iy < height; iy++ {
			// Build face slice
			s := newVoxFaceSlice(width, depth, empty, f)
			for iz := 0; iz < depth; iz++ {
				for ix := 0; ix < width; ix++ {
					vv := v.Get(ix, iy, iz)
					if vv == empty {
...
type verter[T any] interface {
	...
	vert(x, y, z uint8, c T, f Facing)
}

func addFace[T any](p, d [3]uint8, f Facing, c T, m verter[T]) {
	d[0] -= 1
	d[1] -= 1
	d[2] -= 1
	switch f {
